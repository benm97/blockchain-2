...
class VirtualBlockchain:
    def __init__(self, initial_blockchain: List[Block], initial_utxo: List[Transaction],
                 spent_txs: Dict[TxID, Transaction]) -> None:
        self.blockchain: List[Block] = initial_blockchain
        self.utxo: List[Transaction] = initial_utxo
        self.spent_txs: Dict[TxID, Transaction] = spent_txs
    ...
    def attempt_reorg(self, new_block_hash: BlockHash, sender: 'Node') -> List[Block]:
        split_hash: Optional[BlockHash] = self.get_split_hash(new_block_hash, sender)
        if self.new_chain:
            ...
            if len(self.new_chain) > len(self.old_chain):
                ...
                prev_block_hash: BlockHash = GENESIS_BLOCK_PREV
                if self.blockchain:
    ...
    def get_split_hash(self, block_hash: BlockHash, sender: 'Node') -> Optional[BlockHash]:
        ...
        while not self.is_known_block(current_block_hash):
            try:
                ...
                if not unknown_block.get_block_hash() == current_block_hash:
                    raise ValueError
    ...
    def compute_old_chain_until(self, split_block_hash: BlockHash) -> None:
        if not self.blockchain:
            return
        current_block_hash: BlockHash = self.blockchain[-1].get_block_hash()
        while not current_block_hash == GENESIS_BLOCK_PREV and not current_block_hash == split_block_hash:
            block: Block = self.get_block(current_block_hash)
            self.old_chain.append(block)
            current_block_hash = block.get_prev_block_hash()

    def validate_block(self, block: Block) -> bool:
        if len(block.get_transactions()) > BLOCK_SIZE:
            return False
        new_coin_counter: int = 0
    ...
    def rollback_block(self, block: Block) -> None:
        for canceled_tx in block.get_transactions():
            ...
            if canceled_tx.input and self.spent_txs[canceled_tx.input]:
                self.utxo.append(self.spent_txs[canceled_tx.input])
